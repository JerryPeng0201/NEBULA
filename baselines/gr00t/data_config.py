...
class CustomPandaHDF5DataConfig(BaseDataConfig):
    """Data configuration for Panda robot with HDF5 dataset format."""
    
    # Map to your actual camera names in HDF5
    video_keys = [
        "video.base_camera",
        "video.hand_camera", 
        # Add more cameras as needed: "video.back_right_camera", etc.
    ]
    
    # Map to your state structure  
    state_keys = [
        "state.single_arm",    # qpos from HDF5
        "state.gripper",   # qvel from HDF5
    ]
    
    # Map to your action structure
    action_keys = [
        "action.single_arm",     # actions from HDF5
        "action.gripper",     # actions from HDF5
    ]
    ...
    # Temporal sampling
    observation_indices = [0]           # Current timestep only
    action_indices = list(range(16))    # Next 16 action steps
    
    # Normalization and rotation configurations
    state_normalization_modes = {
    ...
    action_normalization_modes = {
        ...
        "action.gripper": "binary",  # Gripper open/close
    }
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            # GR00T model-specific transform
            GR00TTransform(
                state_horizon=len(self.observation_indices),
                action_horizon=len(self.action_indices),
                max_state_dim=64,
...
class OxeDroidDataConfig(BaseDataConfig):
    ...
    language_keys = ["annotation.language.task_instruction"]
    observation_indices = [0]
    action_indices = list(range(16))
...
DATA_CONFIG_MAP = {
    ...
    "custom_panda_hdf5": CustomPandaHDF5DataConfig(),
