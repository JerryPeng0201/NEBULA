...
@dataclass
class ArgsConfig:
    # Replace single task parameters with multiple tasks
    with open('config.yaml', 'r') as f:
        config = yaml.safe_load(f)
    dataset_root: str = config['experiment']['dataset_root']
    """Root path to the HDF5 dataset."""
    
    task_names: List[str] = field(default_factory=lambda: ["Control-PlaceSphere-Easy", "Control-PushCube-Easy", "Control-StackCube-Easy", 
    ...
    subtask_name: str = "0"
    """Subtask folder name (same for all tasks)."""
    
    # Add task weighting option
    task_weights: List[float] = None
    """Weights for each task. If None, uses equal weights."""

    output_dir: str = "/HDD1/embodied_ai/open_source_test"
    """Directory to save model checkpoints."""

    use_all_tasks: bool = True
    """Whether to use all available tasks and subtasks in the dataset root."""

    data_config: str = "custom_panda_hdf5"
    """
...
def discover_all_datasets(dataset_root: str) -> List[Tuple[str, str]]:
    """Discover all task/subtask combinations in the dataset root.
    
    Returns:
        List of (task_name, subtask_name) tuples
    """
    import os
    from pathlib import Path
    ...
    # Scan for all task folders
    for task_folder in dataset_path.iterdir():
        if not task_folder.is_dir():
            continue
            
        # Check if it has motionplanning subfolder
        motionplanning_path = task_folder / "motionplanning"
        if not motionplanning_path.exists():
            continue
            
        # Scan for all subtask folders
        for subtask_folder in motionplanning_path.iterdir():
            if not subtask_folder.is_dir():
                continue
                
            # Check if it has the required files (h5 files)
            h5_files = list(subtask_folder.glob("*.h5"))
            if len(h5_files) > 0:
                task_subtask_pairs.append((task_folder.name, subtask_folder.name))
    
    print(f"Discovered {len(task_subtask_pairs)} task/subtask combinations:")
    for task, subtask in task_subtask_pairs:
        print(f"  - {task}/{subtask}")
    
    return task_subtask_pairs
...
def main(config: ArgsConfig):
    ...
    # 1.1 Use custom HDF5 data config
    data_config_cls = CustomPandaHDF5DataConfig()
    modality_configs = data_config_cls.modality_config()
    transforms = data_config_cls.transform()

    # 1.2 Discover all task/subtask combinations
    task_subtask_pairs = discover_all_datasets(config.dataset_root)

    if len(task_subtask_pairs) == 0:
        raise ValueError(f"No valid task/subtask combinations found in {config.dataset_root}")

    # 1.3 Create HDF5 datasets for each task/subtask combination
    single_datasets = []
    for task_name, subtask_name in task_subtask_pairs:
        try:
            dataset = HDF5LeRobotDataset(
                ...
                camera_names=["base_view", "hand_view", "wrist_view"],
            ...
            print(f"Loaded dataset: {task_name}/{subtask_name} ({len(dataset)} samples)")
        except Exception as e:
            print(f"Failed to load {task_name}/{subtask_name}: {e}")
            continue

    if len(single_datasets) == 0:
        raise ValueError("No datasets could be loaded successfully")

    # 1.4 Create mixture dataset
    train_dataset = LeRobotMixtureDataset(
        data_mixture=[
            (dataset, 1.0)  # Equal weights for all task/subtask combinations
    ...
    print(f"Created mixture dataset with {len(single_datasets)} task/subtask combinations")
    print(f"Total samples across all datasets: {len(train_dataset)}")
