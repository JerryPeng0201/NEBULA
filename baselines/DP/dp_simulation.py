...
# Fix for nebula agent utils
import nebula.core.embodiment.utils as na_utils
_na_get_idx = na_utils.get_active_joint_indices

def _get_idx_long(articulation, joint_names):
    return _na_get_idx(articulation, joint_names).long()

na_utils.get_active_joint_indices = _get_idx_long
...
def create_diffusion_policy(config):
...
def nebula_to_dp_obs(env, obs, obs_window, state_min, state_max):
    """Convert Nebula observation to Diffusion Policy format."""
...
def dp_to_nebula_action(dp_action, action_min, action_max):
    """Convert Diffusion Policy action to Nebula format."""
    actions = dp_action['action_pred'].squeeze(0)
    # Denormalize actions
    actions = (actions + 1) / 2 * (action_max - action_min) + action_min
    actions = actions.detach().cpu().numpy()
    actions = actions[:8]  # Take first 8 actions
...
def run_episode(env, policy, env_id, config, episode_idx, save_video=False, video_dir=None):
    ...
    state_min = torch.tensor(config['model']['data_stat']['state_min']).cuda()
    state_max = torch.tensor(config['model']['data_stat']['state_max']).cuda()
    action_min = torch.tensor(config['model']['data_stat']['action_min']).cuda()
    action_max = torch.tensor(config['model']['data_stat']['action_max']).cuda()
    
    initial_obs = nebula_to_dp_obs(env, obs, obs_window, state_min, state_max)
    obs_window.append(initial_obs)
    if len(obs_window) < 2:
        obs_window.append(initial_obs)
    ...
    while global_steps < config['experiment']['max_episode_steps'] and not done:
        ...
        for action in nebula_actions:
            ...
            obs = nebula_to_dp_obs(env, obs, obs_window, state_min, state_max)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config_dp.yaml')
