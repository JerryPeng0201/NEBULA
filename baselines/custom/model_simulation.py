...
def model_to_nebula_action(gr00t_action):
...
def run_episode(env, policy, env_id, config, episode_idx, save_video=False, video_dir=None):
    ...
    while global_steps < config['experiment']['max_episode_steps'] and not done:
        ...
        model_obs = nebula_to_model_obs(obs, task_description, info)
        start_time = time.perf_counter()
        
        # --- modify here based on your policy's API ---
        model_action = policy.get_action(model_obs) # you may need to modify this line based on your policy's API
        # --- end modification ---
        ...
        nebula_actions = model_to_nebula_action(model_action)
...
def main():
    ...
    policy = create_model_policy(config)
