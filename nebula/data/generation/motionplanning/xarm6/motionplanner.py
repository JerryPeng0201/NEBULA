...
OPEN = 0       # G2 outer_knuckle at 0 = fingers open
CLOSED = 0.85  # G2 outer_knuckle at 0.85 = fingers closed

# Number of arm joints (excluding gripper joints)
NUM_ARM_JOINTS = 6


def _to_mplib_pose(sapien_pose):
    """Convert a sapien Pose to an mplib Pose."""
    return mplib.Pose(p=sapien_pose.p, q=sapien_pose.q)
...
class XArmMotionPlanningSolver:
    ...
    def _get_arm_qpos(self):
        """Get only the arm joint positions (first 6), excluding gripper joints."""
        return self.robot.get_qpos().cpu().numpy()[0, :NUM_ARM_JOINTS]
    ...
    def setup_planner(self):
        ...
        planner = mplib.Planner(
            ...
            move_group="eef",
            joint_vel_limits=np.ones(NUM_ARM_JOINTS) * self.joint_vel_limits,
            joint_acc_limits=np.ones(NUM_ARM_JOINTS) * self.joint_acc_limits,
        )
        planner.set_base_pose(_to_mplib_pose(self.base_pose))
        return planner
    ...
    def move_to_pose_with_RRTConnect(
        ...
        goal_pose = mplib.Pose(p=pose.p, q=pose.q)
    ...
    def move_to_pose_with_screw(
        ...
        if self.grasp_pose_visual is not None:
            self.grasp_pose_visual.set_pose(pose)
        goal_pose = mplib.Pose(p=pose.p, q=pose.q)
        arm_qpos = self._get_arm_qpos()

        # Try screw plan twice
        ...
        # Fallback to RRT if screw fails
        if result["status"] != "Success":
            result = self.planner.plan_pose(
                goal_pose, arm_qpos,
                time_step=self.base_env.control_timestep,
...
def build_xarm_gripper_grasp_pose_visual(scene: NEBULAScene):
    ...
    builder.add_box_visual(
        ...
        half_size=[0.035, grasp_pose_visual_width, grasp_pose_visual_width],
