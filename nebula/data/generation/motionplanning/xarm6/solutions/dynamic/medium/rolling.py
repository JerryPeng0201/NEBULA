...
from nebula.benchmarks.capabilities.dynamic.medium import DynamicMediumPlaceRollingSphereEnv
from nebula.data.generation.motionplanning.xarm6.motionplanner import (
...
def move_to_pose_with_screw_fast(planner, pose, dry_run=False, speed_multiplier=8.0):
    ...
    arm_qpos = planner._get_arm_qpos()
    ...
    if result["status"] != "Success":
        result = planner.planner.plan_pose(
            goal_pose,
            arm_qpos,
            time_step=fast_time_step,
            wrt_world=True,
...
def DynamicMediumPlaceRollingSphereSolution(env: DynamicMediumPlaceRollingSphereEnv, seed=None, debug=False, vis=False):
    """
    DynamicMedium Rolling Sphere solver: Direct sphere capture and placement.
    
    Strategy:
    1. Track and capture rolling sphere immediately
    2. Place in bin and verify success
    """
    outer_env = env
    env.reset(seed=seed)
    ...
    try:
        if debug:
            print(f"Starting rolling sphere task solution...")
            print(f"Sphere position: {_np_pos(env_u.obj)}")
            print(f"Bin position: {_np_pos(env_u.bin)}")
        # Phase 1: Capture the rolling sphere
        ...
        print(f"Error in DynamicMediumPlaceRollingSphereSolution: {e}")
        if debug:
            import traceback
            traceback.print_exc()
        try:
