...
import random
from nebula.benchmarks.capabilities.perception.easy.pick_sphere import PerceptionPickSphereEasyEnv
...
def PerceptionPickSphereEasySolution(env: PerceptionPickSphereEasyEnv, seed=None, debug=False, vis=False):
    """
    Perception task solution: Only grasp the red sphere.
    No placement is required for perception tasks.
    """
    env.reset(seed=seed, options={"task_instruction": "Grasp the sphere."})
    ...
    # Task complete: the robot has successfully grasped the sphere
    planner.close()
    return res
