...
import random
import torch
from nebula.benchmarks.capabilities.perception.hard.pick_rightCubes import PerceptionPickRightCubesHardEnv
...
def PerceptionPickRightCubesHardSolution(env: PerceptionPickRightCubesHardEnv, seed=None, debug=False, vis=False):
    """
    Perception task solution: Identify and grasp the small cube that fits in the bin.
    No placement is required for perception tasks.
    """
    env.reset(seed=seed, options={"task_instruction": "Grasp the small cube that fits in the bin."})
    ...
    # Target: the small cube (the one that fits in the bin)
    target_cube = env.small_cube
    ...
    if not res:
        print("Failed to reach the small cube")
        planner.close()
        return res
    ...
    if not res:
        print("Failed to lift the small cube")
        planner.close()
        return res

    # Task complete: the robot has successfully grasped the small cube
    planner.close()
    return res
