...
from nebula.utils.geometry.trimesh_utils import get_component_mesh
...
def compute_push_info_by_obb(
    ...
    """Compute push info given an oriented bounding box.
    The push info includes axes to define push frame, namely approaching, closing, orthogonal directions and center.

    Args:
        obb: oriented bounding box to push
        approaching: direction to approach the object
        target_closing: target closing direction, used to select one of multiple solutions
        depth: displacement from hand to tcp along the approaching vector. Usually finger length.
        ortho: whether to orthogonalize closing  w.r.t. approaching.
    ...
    return compute_grasp_info_by_obb(
        obb,
