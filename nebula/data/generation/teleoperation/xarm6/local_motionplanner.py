...
class XArmMotionPlanningSolver:
    ...
    def follow_path(self, result, refine_steps: int = 0):
        ...
        for i in range(n_step + refine_steps):
            ...
            action = self._build_action(qpos, qvel)
    ...
    def move_to_pose_with_screw(
        ...
        pose = sapien.Pose(p=pose.p , q=pose.q)
        print(self.robot.get_qpos().cpu().numpy()[0])
...
def build_panda_gripper_grasp_pose_visual(scene: NEBULAScene):
    ...
    builder.add_box_visual(
        ...
        half_sqposize=[grasp_pose_visual_width, grasp_width, grasp_pose_visual_width],
        material=sapien.render.RenderMaterial(base_color=[0, 1, 0, 0.7]),
    )
    builder.add_box_visual(
        pose=sapien.Pose(
