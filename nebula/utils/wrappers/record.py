...
from nebula import get_commit_info
from nebula.core.simulation.engine import BaseEnv
from nebula.utils import common, gym_utils, sapien_utils
from nebula.utils.io_utils import dump_json
from nebula.utils.logging_utils import logger
from nebula.utils.structs.types import Array
from nebula.utils.visualization.misc import (
    images_to_video,
    put_info_on_image,
    tile_images,
)
from nebula.utils.wrappers import CPUGymWrapper
...
class RecordEpisode(gym.Wrapper):
    ...
    - `videos` (Dict): video paths for each camera view
    ...
    Args:
        ...
        task_name (Optional[str]): Task name for organizing video folders
        subtask_idx (Optional[int]): Subtask index for organizing video folders
    """
    ...
    def __init__(
        ...
        if save_trajectory or save_video:
            ...
            (self.output_dir / str(subtask_idx)).mkdir(parents=True, exist_ok=True)
        ...
        # Task and subtask info for video organization
        ...
        if self.save_trajectory:
            ...
            if task_name is not None:
                self._json_data["task_name"] = task_name
                
        self._save_video = save_video
        self.info_on_video = info_on_video
        
        # Multi-camera support
        self.camera_names = ["base_camera", "hand_camera", "back_right_camera", 
                           "back_left_camera", "front_right_camera", "front_left_camera"]
        self.render_images = {}  # Dictionary to store images by camera
        self.current_episode_videos = {}  # Store video paths for current episode
        ...
        if self.render_substeps:
            ...
            def wrapped_after_simulation_step():
                ...
                if self.save_video:
                    ...
                    camera_images = self.capture_image()
                    for camera_name, camera_img in camera_images.items():
                        self.render_images[camera_name].append(camera_img)
    ...
    def capture_image(self, infos=None):
        ...
        # Define the fixed modality order
        MODALITY_ORDER = ['rgb', 'depth', 'segmentation']
        
        # Calculate expected dimensions
        num_cameras = len(self.camera_names)
        num_modalities = len(MODALITY_ORDER)
        single_image_width = 512
        expected_total_width = num_cameras * num_modalities * single_image_width

        # Split concatenated image into individual cameras
        if len(img.shape) == 4 and img.shape[2] == expected_total_width:
            camera_imgs = {}
            ...
            for cam_idx, camera_name in enumerate(self.camera_names):
                ...
                for mod_idx, modality in enumerate(MODALITY_ORDER):
                    image_index = cam_idx * num_modalities + mod_idx
                    start_x = image_index * single_image_width
                    end_x = start_x + single_image_width
                    modality_img = img[0, :, start_x:end_x, :]
                    
                    if infos is not None and modality == 'rgb':
                        info_item = {k: v if np.size(v) == 1 else v[0] for k, v in infos.items()}
                        modality_img = put_info_on_image(modality_img, info_item)
                    
                    camera_data[modality] = modality_img
                
                camera_imgs[camera_name] = camera_data
            
            return camera_imgs
        
        # Fallback for single camera or different concatenation
        # FIXED: Return proper format with modality dict
        single_img = img[0] if len(img.shape) == 4 else img
        
        # Check if image is RGB (3 channels) or single channel
        if len(single_img.shape) == 3 and single_img.shape[2] == 3:
            # RGB image
            return {"single_camera": {"rgb": single_img}}
        else:
            # Assume it's a depth or segmentation image
            return {"single_camera": {"depth": single_img}}
    ...
    def reset(
        ...
        **kwargs,
        ...
        self.current_episode_task_instruction = None
        if options and "task_instruction" in options:
            self.current_episode_task_instruction = options["task_instruction"]
            # Remove task_instruction from reset_kwargs to avoid duplication
            filtered_options = {k: v for k, v in options.items() if k != "task_instruction"}
            self.last_reset_kwargs = copy.deepcopy(dict(options=filtered_options, **kwargs))
    ...
    def step(self, action):
        if self.save_video and self._video_steps == 0:
            ...
            camera_images = self.capture_image()
            for camera_name, camera_img in camera_images.items():
                if camera_name not in self.render_images:
                    self.render_images[camera_name] = []
                self.render_images[camera_name].append(camera_img)
        ...
        if self.save_video:
            ...
            if self.info_on_video:
                ...
                camera_images = self.capture_image(scalar_info)
            else:
                camera_images = self.capture_image()
    ...
    def flush_trajectory(
        ...
        for env_idx in env_idxs_to_flush:
            ...
            if save:
                ...
                if hasattr(self, 'current_episode_task_instruction') and self.current_episode_task_instruction is not None:
                    episode_info.update(task_instruction=self.current_episode_task_instruction)

                if self.save_video and hasattr(self, 'previous_episode_videos') and self.previous_episode_videos:
                    episode_info.update(videos=self.previous_episode_videos)
                    # Clear after using
                    self.previous_episode_videos = {}

                if hasattr(self.base_env, 'task_instruction'):
                    episode_info.update(task_instruction=self.base_env.task_instruction)
                elif hasattr(self, 'current_episode_task_instruction') and self.current_episode_task_instruction is not None:
                    episode_info.update(task_instruction=self.current_episode_task_instruction)
                
                if self.save_video and self.current_episode_videos is not None:
                    episode_info.update(videos=self.current_episode_videos)
    ...
    def flush_video(
        ...
        """
        Flush videos for each camera view and save to respective folders
        ...
        """
        if not self.render_images or not any(len(imgs) > 0 for imgs in self.render_images.values()):
            return
            
        if ignore_empty_transition and all(len(imgs) == 1 for imgs in self.render_images.values()):
            return
        ...
        if save:
            ...
            # Save videos for each camera view and modality
            for camera_name, frame_list in self.render_images.items():
                # print(camera_name)
                # print(len(frame_list))
                if len(frame_list) == 0:
                    continue
                
                # Reorganize from list of frame dicts to dict of modality lists
                modality_data = {}
                # print(type(frame_list))
                for frame_dict in frame_list:
                    # print(frame_dict)
                    for modality, frame_img in frame_dict.items():
                        if modality not in modality_data:
                            modality_data[modality] = []
                        modality_data[modality].append(frame_img)
                
                # Now save videos for each modality
                for modality, images in modality_data.items():
                    if len(images) == 0:
                        continue
                        
                    # Create folder structure: TaskName/SubtaskIdx/CameraView/Modality/
                    camera_view_name = camera_name.replace("_camera", "_view")
                    video_dir = self.output_dir / str(self.subtask_idx) / camera_view_name / modality
                    video_dir.mkdir(parents=True, exist_ok=True)
                    
                    # Check for existing files if avoiding overwrite
                    final_video_name = video_name
                    if self._avoid_overwriting_video:
                        counter = 0
                        while (video_dir / (final_video_name.replace(" ", "_").replace("\n", "_") + ".mp4")).exists():
                            counter += 1
                            final_video_name = f"{video_name}_{counter}"
                    
                    # Convert single-channel images to 3-channel for video if needed
                    ...
                    # Store relative video path for JSON
                    relative_path = f"{self.task_name}/{self.subtask_idx}/{camera_view_name}/{modality}/{final_video_name}.mp4"
                    video_key = f"{camera_name}_{modality}"
                    self.current_episode_videos[video_key] = relative_path
                    
                    if verbose:
                        print(f"Saved {camera_name} {modality} video: {relative_path}")
