...
from nebula.utils.structs.types import Array, Device
...
def to_sapien_pose(pose: Union[torch.Tensor, sapien.Pose, Pose]) -> sapien.Pose:
    ...
    if isinstance(pose, sapien.Pose):
        ...
        ), "pose is batched. Note that sapien Poses are not batched. If you want to use a batched Pose object use from nebula.utils.structs.pose import Pose"
        if len(pose.shape) == 2:
            pose = pose[0]
