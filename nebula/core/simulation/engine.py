...
import nebula.core.render.utils as render_utils
from nebula import logger
from nebula.core.embodiment import REGISTERED_AGENTS
from nebula.core.embodiment.manipulator import BaseAgent
from nebula.core.embodiment.multi_manipulator import MultiAgent
from nebula.core.simulation.scene import NEBULAScene
from nebula.core.simulation.utils.observations import (
    parse_obs_mode_to_struct,
    sensor_data_to_pointcloud,
)
from nebula.core.simulation.utils.randomization.batched_rng import BatchedRNG
from nebula.core.simulation.utils.system.backend import (
    CPU_SIM_BACKENDS,
    parse_sim_and_render_backend,
)
from nebula.core.sensors.base_sensor import BaseSensor, BaseSensorConfig
from nebula.core.sensors.camera import (
    Camera,
    CameraConfig,
    parse_camera_configs,
    update_camera_configs_from_dict,
)
from nebula.core.sensors.depth_camera import StereoDepthCamera, StereoDepthCameraConfig
from nebula.utils import common, gym_utils, sapien_utils
from nebula.utils.structs import Actor, Articulation
from nebula.utils.structs.pose import Pose
from nebula.utils.structs.types import Array, SimConfig
from nebula.utils.visualization.misc import tile_images
from nebula.tools.viewer import create_viewer
...
class BaseEnv(gym.Env):
    ...
    def _get_obs_sensor_data(self, apply_texture_transforms: bool = True) -> dict:
        ...
        MODALITY_ORDER = ["rgb", "depth", "segmentation", "position", "normal", "albedo"]
        ordered_sensor_obs = dict()
        for sensor_name, sensor_data in sensor_obs.items():
            ordered_data = dict()
            # Add modalities in the fixed order
            for modality in MODALITY_ORDER:
                if modality in sensor_data:
                    ordered_data[modality] = sensor_data[modality]
            ordered_sensor_obs[sensor_name] = ordered_data
