...
@register_env("Perception-PlaceSphere-Easy", max_episode_steps=50)
class PerceptionPlaceSphereEasyEnv(BaseEnv):
    """
    **Task Description:**
    Place either object (blue cube or red sphere) into the shallow bin.

    **Randomizations:**
    - The position of the bin and the objects are randomized: The bin is initialized in [0, 0.1] x [-0.1, 0.1],
    and the objects are initialized in fixed positions with some distance between them.

    **Success Conditions:**
    - Either object is placed on the top of the bin. The robot remains static and the gripper is not closed at the end state.
    """
    ...
    cube_half_size = 0.02  # half size of the cube
    ...
    def _load_scene(self, options: dict):
        ...
        # load the blue cube and red sphere
        self.blue_cube = actors.build_cube(
            ...
            name="blue_cube",
            body_type="dynamic",
            initial_pose=Pose.create_from_pq(p=[0, 0, self.cube_half_size], q=[1, 0, 0, 0])
        )
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # init the objects in the first 1/4 zone along the x-axis (so that they don't collide the bin)
            xyz = torch.zeros((b, 3))
            q = [1, 0, 0, 0]
    ...
    def evaluate(self):
        ...
        # Check if either object is on the bin
        is_blue_cube_on_bin = self._check_cube_on_bin(pos_blue_cube, pos_bin)
        is_red_sphere_on_bin = self._check_sphere_on_bin(pos_red_sphere, pos_bin)
        is_any_object_on_bin = torch.logical_or(is_blue_cube_on_bin, is_red_sphere_on_bin)
        ...
        # Success: either object on bin, that object is static, and that object is not grasped
        ...
        success = torch.logical_or(blue_cube_success, red_sphere_success)
    ...
    def _check_cube_on_bin(self, pos_cube, pos_bin):
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        # reaching reward - reward for getting close to either object
        ...
        bin_top_pos_cube[:, 2] = bin_top_pos_cube[:, 2] + self.block_half_size[0] + self.cube_half_size
        
        bin_top_pos_sphere = self.bin.pose.p.clone()
        bin_top_pos_sphere[:, 2] = bin_top_pos_sphere[:, 2] + self.block_half_size[0] + self.radius
        
        blue_cube_to_bin_dist = torch.linalg.norm(bin_top_pos_cube - blue_cube_pos, axis=1)
        red_sphere_to_bin_dist = torch.linalg.norm(bin_top_pos_sphere - red_sphere_pos, axis=1)
        
        # Reward based on whichever object is grasped
        is_blue_grasped = info["is_blue_cube_grasped"]
        is_red_grasped = info["is_red_sphere_grasped"]
        
        blue_place_reward = 1 - torch.tanh(5.0 * blue_cube_to_bin_dist)
        ...
        is_any_object_grasped = torch.logical_or(is_blue_grasped, is_red_grasped)
        ungrasp_reward = (
        ...
        blue_v = torch.linalg.norm(self.blue_cube.linear_velocity, axis=1)
        blue_av = torch.linalg.norm(self.blue_cube.angular_velocity, axis=1)
        ...
        is_blue_on_bin = info["is_blue_cube_on_bin"]
