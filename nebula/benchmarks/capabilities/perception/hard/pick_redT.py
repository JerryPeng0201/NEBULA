...
@register_env("Perception-PickRedT-Hard", max_episode_steps=50)
class PerceptionPickRedTHardEnv(BaseEnv):
    """
    **Task Description:**
    Grasp the red T-shape among six objects of different shapes and colors (blue sphere, green cube, 
    red T-shape, red L-shape, yellow cube, and red sphere).
    ...
    **Randomizations:**
    - The objects are placed at fixed positions in a row at x=-0.08, with y positions ranging 
    from -0.20 to 0.20, spaced 0.08 units apart for better separation.
    ...
    **Success Conditions:**
    ...
    def create_small_tee(self, name="small_tee", target=False, base_color=None):
        ...
        # Set initial pose to match _initialize_episode position
        builder.initial_pose = sapien.Pose(p=[-0.08, -0.04, 0.012], q=[1, 0, 0, 0])
        if not target:
    ...
    def _load_scene(self, options: dict):
        ...
        # load the six objects (with better spacing - 0.08 units apart)
        self.blue_sphere = actors.build_sphere(
            ...
            name="blue_sphere",
            body_type="dynamic",
            initial_pose=Pose.create_from_pq(p=[-0.08, -0.20, self.object_size], q=[1, 0, 0, 0])
        )
        ...
        self.green_cube = actors.build_cube(
            ...
            name="green_cube",
            body_type="dynamic",
            initial_pose=Pose.create_from_pq(p=[-0.08, -0.12, self.object_size], q=[1, 0, 0, 0])
        )

        # load the red T-shape (bigger now)
        self.red_tee = self.create_small_tee(name="red_tee", target=False)

        # load the L-shape (bigger now)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # Object positions (6 objects in a single line with better spacing)
            # All objects on same x-coordinate, spaced 0.08 units apart in y-direction
            
            # Blue sphere
            xyz[..., 0] = -0.08  # fixed x position for all objects
            xyz[..., 1] = -0.20  # leftmost position
            ...
            # Green cube
            xyz[..., 0] = -0.08
            xyz[..., 1] = -0.12  # spaced 0.08 units apart
            xyz[..., 2] = self.object_size
            green_cube_pose = Pose.create_from_pq(p=xyz.clone(), q=q)
            self.green_cube.set_pose(green_cube_pose)

            # Red T-shape (bigger now - scale_factor 0.6)
            xyz[..., 0] = -0.08
            xyz[..., 1] = -0.04  # spaced 0.08 units apart
            ...
            # L-shape (bigger now - scale_factor 0.6)
            xyz[..., 0] = -0.08
            xyz[..., 1] = 0.07   # spaced 0.08 units apart
            xyz[..., 2] = 0.012  # L thickness/2 (0.04/2 * 0.6 = 0.012)
            l_shape_pose = Pose.create_from_pq(p=xyz.clone(), q=q)
            self.l_shape.set_pose(l_shape_pose)

            # Yellow cube
            xyz[..., 0] = -0.08
            xyz[..., 1] = 0.15   # spaced 0.08 units apart
            xyz[..., 2] = self.object_size
