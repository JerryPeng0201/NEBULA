...
@register_env("Robust-StackCube-Medium", max_episode_steps=250)
class RobustStackCubeMediumEnv(BaseEnv):
    """
    **Task Description:**
    - The goal is to arrange four colored cubes: place the yellow cube next to the blue cube, stack the red cube on top of both yellow and blue cubes, and place the green cube next to the arrangement.
    ...
    **Success Conditions:**
    - the red cube is static and on top of both the yellow and blue cube
    - the green cube is placed next to the arrangement
    ...
    def _load_scene(self, options: dict):
        ...
        self.scene, half_size=0.02, color=[1, 1, 0, 1], name="cubeA", initial_pose=sapien.Pose(p=[0, 0, 0.2])
        )  # Yellow
        
        self.cubeB = actors.build_cube(
            self.scene, half_size=0.02, color=[0, 0, 1, 1], name="cubeB", initial_pose=sapien.Pose(p=[1, 0, 0.2])
        )  # Blue
        
        self.cubeC = actors.build_cube(
            self.scene, half_size=0.02, color=[1, 0, 0, 1], name="cubeC", initial_pose=sapien.Pose(p=[-1, 0, 0.2])
        )  # Red
        
        self.cubeD = actors.build_cube(
            self.scene, half_size=0.02, color=[0, 1, 0, 1], name="cubeD", initial_pose=sapien.Pose(p=[0, 1, 0.2])
        )  # Green
    ...
    def compute_dense_reward(self, obs: Dict, action: torch.Tensor, info: Dict):
        ...
        # Get cube positions
        pos_A = self.cubeA.pose.p  # Yellow cube (was Red)
        pos_B = self.cubeB.pose.p  # Blue cube (was Green)
        pos_C = self.cubeC.pose.p  # Red cube (was Blue)
        pos_D = self.cubeD.pose.p  # Green cube (was Purple)
