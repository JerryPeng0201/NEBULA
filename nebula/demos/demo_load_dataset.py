#!/usr/bin/env python3
"""
NEBULA Dataset Loader Script
Follows GROOT dataset loading logic adapted for NEBULA dataset.
"""
...
def get_dataset_info(dataset: NEBULADataset) -> dict:
    """
    Get comprehensive information about the NEBULA dataset.
    """
    stats = dataset.get_dataset_statistics()
    
    # Get robot configuration info
    config = dataset.robot_config
    robot_info = {
...
def analyze_episode_data(episode, episode_idx: int, config: Embodiment):
    """
...
def extract_trajectories(dataset: NEBULADataset, num_episodes: int, config: Embodiment):
    """
    Extract state and action trajectories from multiple episodes.
    """
    state_trajectories = {limb: [] for limb in config.get_all_state_limbs()}
    action_trajectories = {limb: [] for limb in config.get_all_action_limbs()}
    camera_images = []
    instructions = []
    ...
    for i in range(min(num_episodes, len(dataset))):
        ...
        episode_states = {limb: [] for limb in config.get_all_state_limbs()}
        episode_actions = {limb: [] for limb in config.get_all_action_limbs()}
        episode_images = []
...
def load_nebula_dataset(
    ...
    """
    Load and visualize NEBULA dataset following GROOT loading pattern.
    """
    
    print("="*100)
    print(f"{'NEBULA Dataset Loading':=^100}")
    print("="*100)
    
    # 1. Initialize dataset
    print_blue("Initializing NEBULA dataset...")
    ...
    print_blue("Analyzing dataset structure...")
    ...
    # 3. Analyze first episode in detail
    if len(dataset) > 0:
        print_blue("Analyzing first episode...")
        first_episode = dataset[0]
        analyze_episode_data(first_episode, 0, dataset.robot_config)
    
    # 4. Extract and visualize trajectories
    ...
    state_trajectories, action_trajectories, camera_images, instructions = extract_trajectories(
        dataset, num_episodes, dataset.robot_config
    )
    ...
    # 5. Plot state and action trajectories
    if plot_state_action and any(traj.size > 0 for traj in state_trajectories.values()):
        ...
        valid_limbs = [limb for limb in dataset.robot_config.get_all_state_limbs() 
    ...
    print_blue("Dataset usage examples:")
    print("""
# Basic usage:
episode = dataset[0]
first_step = episode.steps[0]
...
# Get robot state
arm_state = first_step.observation.robot_state.limbs['single_arm'].joint_positions
gripper_state = first_step.observation.robot_state.limbs['gripper'].joint_positions

# Get actions  
arm_action = first_step.action.robot_action.limbs['single_arm']
gripper_action = first_step.action.robot_action.limbs['gripper']

# Get camera images
base_camera_rgb = first_step.observation.image['base_camera']

# Get instruction
instruction = episode.instruction.raw_text

# Filter by task
pick_episodes = dataset.get_episodes_by_task('Control-PlaceSphere-Easy')
    """)
    
    print("="*100)
    print(f"{'Dataset Loading Complete!':=^100}")
    print("="*100)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print_yellow(f"Successfully loaded dataset with {len(dataset)} episodes!")
